test_plotFITS <- function(pedigree.names, input.dir, output.dir, out.name, 
  ...
  gg <- gg + labs(x="delta t (generations)", y="methylation divergence", color="Genotypes") 
  
  ymax <- max(df_div$div.obs)
  xmax <- max(df_div$delta.t)
  ...
  #colormodel = 'cmyk'
  gg <- gg + labs(x=expression(paste(Delta,"t (generations)")), y="methylation divergence", color="Genotypes",title=Genotypes_2)
  ...
  # 指定颜色分别为 purple 和 red
  gg <- gg + scale_color_manual(values = c("purple", "red"))
